...
app.get('/posts/:id', (req, res) => {
  ...
  posts.get(req.token, req.params.id).then(
    data => {
      log('success', `Details of the post have been provided`)
      return res.send(data)
    },
    error => {
      log('error', error)
...
app.get('/posts/:id/comments', (req, res) => {
  ...
  comments.getByParent(req.token, req.params.id).then(
    data => {
      log(
        'success',
        `All (${data.length}) the comments of the post have been provided`
      )
      return res.send(data)
    },
    error => {
...
app.get('/comments/:id', (req, res) => {
  ...
  comments.get(req.token, req.params.id).then(
    data => {
      log(
        'success',
        `Details of the author's comment ${data.author} were provided`
      )
      return res.send(data)
    },
    error => {
...
app.post('/comments/:id', bodyParser.json(), (req, res) => {
  ...
  comments.vote(req.token, req.params.id, option).then(
    data => {
      log(
        'success',
        `The comment of the author ${data.author} has been '${option}'`
      )
      return res.send(data)
    },
    error => {
...
app.delete('/comments/:id', (req, res) => {
  ...
  comments.disable(req.token, req.params.id).then(
    data => {
      log(
        'success',
        `The comment of the author ${data.author} has been deleted`
      )
      return res.send(data)
    },
    error => {
